...
use alloy::primitives::{B256, U256, Bytes, b256};
...
const CLOB_ATTESTATION_MESSAGE: &str = "This message attests that I control the given wallet";
const POLY_PROXY_FACTORY_ADDRESS: &str = "0xaB45c5A4B0c941a2F231C04C3f49182e1A254052";
const POLY_PROXY_INIT_CODE_HASH: B256 =
    b256!("0xd21df8dc65880a8606f09fe0ce3df9b8869287ab0b058be05aa9e8af6330a00b");
const POLY_SAFE_FACTORY_ADDRESS: &str = "0xaacFeEa03eb1561C4e67d661e40682Bd20E3541b";
const POLY_SAFE_INIT_CODE_HASH: B256 =
...
fn derive_proxy_wallet_for_signer(signer: AlloyAddress) -> Result<AlloyAddress> {
    let factory = AlloyAddress::from_str(POLY_PROXY_FACTORY_ADDRESS)
        .context("Invalid Polymarket proxy factory address")?;
    let salt = keccak256(signer.as_slice());
    Ok(factory.create2(salt, POLY_PROXY_INIT_CODE_HASH))
}

/// Derive deterministic Polymarket Safe wallet for a signer EOA on Polygon.
fn derive_safe_wallet_for_signer(signer: AlloyAddress) -> Result<AlloyAddress> {
    let factory = AlloyAddress::from_str(POLY_SAFE_FACTORY_ADDRESS)
        .context("Invalid Polymarket safe factory address")?;
    // Salt is keccak256(ABI(address)) where address is left-padded to 32 bytes.
    let mut padded = [0u8; 32];
    padded[12..].copy_from_slice(signer.as_slice());
    let salt = keccak256(padded);
    Ok(factory.create2(salt, POLY_SAFE_INIT_CODE_HASH))
}
...
pub struct PolymarketApi {
    ...
    api_key_nonce: u64,
    api_credentials: Arc<tokio::sync::Mutex<Option<ApiCredentials>>>,
...
impl PolymarketApi {
    pub fn new(
        ...
        Self {
            ...
            signature_type,
            api_key_nonce: std::env::var("POLYMARKET_API_KEY_NONCE")
                .ok()
                .and_then(|value| value.parse::<u64>().ok())
                .unwrap_or(0),
            api_credentials: Arc::new(tokio::sync::Mutex::new(Self::load_api_credentials_from_env())),
    ...
    fn load_api_credentials_from_env() -> Option<ApiCredentials> {
    ...
    fn signer(&self) -> Result<LocalSigner<alloy::signers::k256::ecdsa::SigningKey>> {
        let private_key = self.private_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!("Private key is required. Set POLYMARKET_PRIVATE_KEY in .env")
        })?;
    ...
    fn signer_address(&self) -> Result<AlloyAddress> {
        Ok(self.signer()?.address())
    }

    /// Add concrete context for CLOB signer validation failures.
    fn clob_signer_error_hint(&self, err: &anyhow::Error) -> String {
        let s = err.to_string();
        if !s.contains("invalid: signer") {
            return s;
        }
        let signer = self
            .signer_address()
        ...
        format!(
            "{s}\n   Signer diagnostics: signature_type={sig}, signer={signer}, proxy_wallet_address={proxy}\n   Fix: for browser-wallet MetaMask + proxy in this SDK, use signature_type=2 (GnosisSafe mode) and ensure signer/proxy mapping is correct."
        )
    }

    async fn get_server_time(&self) -> Result<u64> {
        let url = format!("{}/time", self.clob_url);
        let response = self.client.get(&url).send().await.context("Failed to fetch CLOB server time")?;
        let status = response.status();
        let payload: Value = response.json().await.context("Failed to parse CLOB server time response")?;
        if !status.is_success() {
            anyhow::bail!("Failed to fetch CLOB server time (status {}): {}", status, payload);
        }
        payload
            .as_u64()
            .or_else(|| payload.get("time").and_then(|v| v.as_u64()))
            .or_else(|| payload.get("timestamp").and_then(|v| v.as_u64()))
            .or_else(|| payload.as_str().and_then(|v| v.parse::<u64>().ok()))
            .ok_or_else(|| anyhow::anyhow!("Unexpected /time response: {}", payload))
    }

    async fn create_l1_headers(&self) -> Result<[(String, String); 4]> {
        ...
        let typed_data: TypedData = serde_json::from_value(serde_json::json!({
            ...
            "message": {
                ...
                "nonce": self.api_key_nonce,
                "message": CLOB_ATTESTATION_MESSAGE
            }
        }))
        .context("Failed to build CLOB L1 typed data")?;
        let typed_data_hash = typed_data
            .eip712_signing_hash()
            .context("Failed to hash CLOB L1 authentication payload")?;
        let signature = signer
            .sign_hash_sync(&typed_data_hash)
            .context("Failed to sign CLOB L1 authentication payload")?;
        Ok([
            ("POLY_ADDRESS".to_string(), signer.address().to_string()),
            ("POLY_SIGNATURE".to_string(), signature.to_string()),
            ("POLY_TIMESTAMP".to_string(), timestamp.to_string()),
            ("POLY_NONCE".to_string(), self.api_key_nonce.to_string()),
        ])
    }

    async fn request_api_credentials(&self, method: reqwest::Method, path: &str) -> Result<ApiCredentials> {
        let url = format!("{}{}", self.clob_url, path);
        let mut request = self.client.request(method, &url);
        for (header, value) in self.create_l1_headers().await? {
            request = request.header(header, value);
        }
        let response = request.send().await.context("Failed to request Polymarket API credentials")?;
        let status = response.status();
        let body: Value = response.json().await.context("Failed to parse API credentials response")?;
        if !status.is_success() {
            anyhow::bail!("CLOB auth endpoint {} failed (status {}): {}", path, status, body);
        }
        Ok(ApiCredentials {
            key: body.get("apiKey").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            secret: body.get("secret").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
            passphrase: body.get("passphrase").and_then(|v| v.as_str()).unwrap_or_default().to_string(),
        })
    }

    async fn get_or_create_api_credentials(&self) -> Result<ApiCredentials> {
        if let Some(existing) = self.api_credentials.lock().await.clone() {
            return Ok(existing);
        }

        let derived = self.request_api_credentials(reqwest::Method::GET, "/auth/derive-api-key").await;
        let credentials = match derived {
            Ok(creds) if !creds.key.is_empty() => creds,
            Ok(_) => self.request_api_credentials(reqwest::Method::POST, "/auth/api-key").await?,
            Err(err) => {
                let create = self.request_api_credentials(reqwest::Method::POST, "/auth/api-key").await;
                match create {
                    Ok(creds) => creds,
                    Err(create_err) => {
                        return Err(anyhow::anyhow!(
                            "Failed to derive or create Polymarket API credentials.\nDerive error: {}\nCreate error: {}",
                            err,
                            create_err
                        ));
                    }
        ...
        if credentials.key.is_empty() || credentials.secret.is_empty() || credentials.passphrase.is_empty() {
            anyhow::bail!("Polymarket returned incomplete API credentials");
        }

        *self.api_credentials.lock().await = Some(credentials.clone());
        Ok(credentials)
    }
    ...
    async fn ensure_clob_client(&self) -> Result<u64> {
        let mut guard = self.clob_client_handle.lock().await;
        if let Some(h) = *guard {
            return Ok(h);
        }
        let private_key = self.private_key.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Private key is required. Set POLYMARKET_PRIVATE_KEY in .env"))?;
        let credentials = self.get_or_create_api_credentials().await?;
        let sig_type = match (self.proxy_wallet_address.is_some(), self.signature_type) {
            ...
            (true, None) => {
                eprintln!("⚠️  proxy_wallet_address set but signature_type not specified; defaulting to 2 (browser-wallet proxy/safe mode)");
                2u8
            }
            (true, Some(1)) | (true, Some(2)) => self.signature_type.unwrap(),
            (false, Some(0)) | (false, None) => 0u8,
            // Allow proxy/safe sig types without explicit funder; SDK can derive deterministic wallet.
            (false, Some(1)) | (false, Some(2)) => self.signature_type.unwrap(),
            (_, Some(n)) => anyhow::bail!("Invalid signature_type: {}. Must be 0, 1, or 2", n),
        };
        // For proxy/safe modes, show an advisory mismatch check but do NOT hard-fail:
        // some valid accounts use explicit mappings that differ from deterministic derivation.
        let signer = self.signer_address()?;
        let funder_owned = if sig_type == 1 || sig_type == 2 {
            // Match SDK semantics:
            // - explicit funder/proxy address takes precedence
            // - only auto-derive when explicit funder is missing
            if let Some(configured) = self.proxy_wallet_address.as_deref() {
                let configured_addr = AlloyAddress::from_str(configured)
                    .context("Invalid proxy_wallet_address format")?;
                // Advisory only; do not override explicitly configured funder.
                let derived = if sig_type == 2 {
                    derive_safe_wallet_for_signer(signer).ok()
                } else {
                    derive_proxy_wallet_for_signer(signer).ok()
                };
                if let Some(d) = derived {
                    if configured_addr != d {
                        eprintln!(
                            "⚠️  proxy_wallet_address {} differs from signer-derived {} {} for signer {}. \
Using configured proxy_wallet_address (SDK-compatible explicit funder behavior).",
                            configured,
                            if sig_type == 2 { "safe" } else { "proxy" },
                            format!("{:#x}", d),
                            format!("{:#x}", signer)
                        );
                    }
                }
                Some(format!("{:#x}", configured_addr))
            } else {
                let derived = if sig_type == 2 {
                    derive_safe_wallet_for_signer(signer)?
                } else {
                    derive_proxy_wallet_for_signer(signer)?
                };
                eprintln!(
                    "ℹ️  signature_type={} with no explicit proxy_wallet_address; using signer-derived {} {}.",
                    sig_type,
                    if sig_type == 2 { "safe" } else { "proxy" },
                    format!("{:#x}", derived)
                );
                Some(format!("{:#x}", derived))
            }
        } else {
            self.proxy_wallet_address.clone()
        };
        let funder = funder_owned.as_deref();
        eprintln!(
            "🔐 CLOB client mode: signature_type={} | signer={} | funder={}",
            sig_type,
            format!("{:#x}", signer),
            funder.unwrap_or("none"),
        );
    ...
    pub async fn authenticate(&self) -> Result<()> {
        let creds = self.get_or_create_api_credentials().await?;
        let _ = self.ensure_clob_client().await?;
        *self.authenticated.lock().await = true;
        let signer = self.signer_address().ok().map(|a| format!("{:#x}", a));
        let sig_type = self.signature_type;
        crate::term_ui::print_auth_success(
            self.proxy_wallet_address.as_deref(),
            creds.key.as_str(),
            signer.as_deref(),
            sig_type,
        );
    ...
    fn generate_signature(
        ...
        let secret_bytes = {
            ...
            if let Ok(bytes) = general_purpose::URL_SAFE.decode(&credentials.secret) {
                bytes
            } else if let Ok(bytes) = general_purpose::STANDARD.decode(&credentials.secret) {
                bytes
            } else {
                credentials.secret.as_bytes().to_vec()
            }
        };
    ...
    async fn add_auth_headers(
        ...
        let signature = self.generate_signature(&credentials, method, path, body, timestamp)?;
    ...
    pub async fn place_order(&self, order: &OrderRequest) -> Result<OrderResponse> {
        ...
        let order_id = clob_sdk::post_limit_order(handle, &order.token_id, &order.side, &order.price, &order.size)
            .map_err(|e| anyhow::anyhow!("{}", self.clob_signer_error_hint(&e)))?;
    ...
    pub async fn cancel_order(&self, order_id: &str) -> Result<()> {
        ...
        request = self.add_auth_headers(request, "DELETE", path, &body_str).await
    ...
    pub async fn set_approval_for_all_clob(&self) -> Result<()> {
        ...
        if let Some(proxy_addr) = &self.proxy_wallet_address {
            ...
            let private_key = self.private_key.as_ref()
                .ok_or_else(|| anyhow::anyhow!("Private key is required for token approval. Set POLYMARKET_PRIVATE_KEY in .env"))?;
    ...
    async fn set_approval_for_all_via_relayer(
        ...
        let response = self.client
            ...
            .header("POLY_BUILDER_API_KEY", &credentials.key)
            .header("POLY_BUILDER_TIMESTAMP", &timestamp_str)
            .header("POLY_BUILDER_PASSPHRASE", &credentials.passphrase)
    ...
    async fn place_order_hmac(&self, order: &OrderRequest) -> Result<OrderResponse> {
        ...
        request = self.add_auth_headers(request, "POST", path, &body).await
    ...
    pub fn has_api_credentials(&self) -> bool {
        self.private_key.is_some() || Self::load_api_credentials_from_env().is_some()
    }
    ...
    pub async fn redeem_tokens(
        ...
        let private_key = self.private_key.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Private key is required for order signing. Set POLYMARKET_PRIVATE_KEY in .env"))?;
