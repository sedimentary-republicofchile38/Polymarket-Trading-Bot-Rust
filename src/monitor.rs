...
impl MarketMonitor {
            ...
            for token in &eth_details.tokens {
                ...
                if outcome_upper.contains("UP") || outcome_upper == "1" {
                    *self.eth_up_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("ETH", true, &token.token_id);
                } else if outcome_upper.contains("DOWN") || outcome_upper == "0" {
                    *self.eth_down_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("ETH", false, &token.token_id);
                }
            }
            ...
            for token in &btc_details.tokens {
                ...
                if outcome_upper.contains("UP") || outcome_upper == "1" {
                    *self.btc_up_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("BTC", true, &token.token_id);
                } else if outcome_upper.contains("DOWN") || outcome_upper == "0" {
                    *self.btc_down_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("BTC", false, &token.token_id);
                }
            }
            ...
            if let Ok(solana_details) = self.api.get_market(&solana_condition_id).await {
                ...
                if outcome_upper.contains("UP") || outcome_upper == "1" {
                    *self.solana_up_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("SOL", true, &token.token_id);
                } else if outcome_upper.contains("DOWN") || outcome_upper == "0" {
                    *self.solana_down_token_id.lock().await = Some(token.token_id.clone());
                    crate::term_ui::print_outcome_token_id("SOL", false, &token.token_id);
                }
            }
                ...
                for token in &xrp_details.tokens {
                    ...
                    if outcome_upper.contains("UP") || outcome_upper == "1" {
                        *self.xrp_up_token_id.lock().await = Some(token.token_id.clone());
                        crate::term_ui::print_outcome_token_id("XRP", true, &token.token_id);
                    } else if outcome_upper.contains("DOWN") || outcome_upper == "0" {
                        *self.xrp_down_token_id.lock().await = Some(token.token_id.clone());
                        crate::term_ui::print_outcome_token_id("XRP", false, &token.token_id);
                    }
                }
    ...
    pub async fn fetch_market_data(&self) -> Result<MarketSnapshot> {
        ...
        let _eth_remaining_str = format_remaining_time(eth_remaining_secs);
        let _btc_remaining_str = format_remaining_time(btc_remaining_secs);
        let _solana_remaining_str = if solana_condition_id == "dummy_solana_fallback" {
            "N/A".to_string()
        } else {
            format_remaining_time(solana_remaining_secs)
        };
        let _xrp_remaining_str = if xrp_condition_id == "dummy_xrp_fallback" {
            "N/A".to_string()
        } else {
        ...
        let mut parts: Vec<String> = vec![
            crate::term_ui::fmt_btc_book(&btc_up_str, &btc_down_str),
        ];
        if self.log_enable_eth {
            parts.push(crate::term_ui::fmt_eth_book(&eth_up_str, &eth_down_str));
        }
        if self.log_enable_solana {
            parts.push(crate::term_ui::fmt_sol_book(
                &solana_up_str,
                &solana_down_str,
            ));
        }
        if self.log_enable_xrp {
            parts.push(crate::term_ui::fmt_xrp_book(&xrp_up_str, &xrp_down_str));
        }
        parts.push(crate::term_ui::fmt_countdown(&time_remaining_str));
        ...
        if let (Some(chainlink_arc), Some(cache_arc)) = (&self.chainlink_btc, &self.price_to_beat_cache) {
            ...
            if let Some(&btc_now) = chainlink.as_ref() {
                ...
                parts.push(crate::term_ui::fmt_chainlink_beat(btc_now, beat));
            }
        }
        ...
        if let Some(ref status_arc) = self.trailing_status_line {
            ...
            if !s.is_empty() {
                parts.push(crate::term_ui::fmt_trailing_status_line(&s));
            }
        }

        let price_log_line = parts.join(&crate::term_ui::pipe_sep_colored());
        let timestamp = Utc::now().format("%Y-%m-%dT%H:%M:%SZ").to_string();
        let plain_entry = format!("[{}] {}\n", timestamp, price_log_line);
        let display_entry = crate::term_ui::format_price_tick_line(&timestamp, &price_log_line);
        crate::log_to_history(&display_entry);
    ...
    {
        crate::term_ui::print_monitoring_start();
