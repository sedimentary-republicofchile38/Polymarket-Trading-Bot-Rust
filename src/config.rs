...
pub struct Args {
    ...
    /// Default: production mode (false); pass --simulation / -s for paper trading
    #[arg(short, long, default_value_t = false)]
    pub simulation: bool,

    /// Run in production mode (execute real trades)
    /// Redundant when simulation already defaults off; kept for explicit scripts
    #[arg(long)]
...
impl Args {
    ...
    /// If --no-simulation is used, forces production
...
pub struct PolymarketConfig {
    ...
    /// Private key for signing orders. Prefer setting `POLYMARKET_PRIVATE_KEY` in `.env`.
    pub private_key: Option<String>,
    /// Proxy wallet address. Prefer setting `POLYMARKET_PROXY_WALLET_ADDRESS` in `.env`.
    pub proxy_wallet_address: Option<String>,
    /// Signature type for authentication. Prefer setting `POLYMARKET_SIGNATURE_TYPE` in `.env`.
    /// 0 = EOA only (no proxy funder; trade directly from the key’s address)
    /// 1 = Proxy wallet mode (Magic/email style proxy)
    /// 2 = Browser wallet proxy/safe mode (MetaMask + Polymarket proxy in this SDK)
...
impl Config {
    ...
    fn apply_env_overrides(&mut self) {
        self.polymarket.private_key = std::env::var("POLYMARKET_PRIVATE_KEY")
            .ok()
            .or_else(|| self.polymarket.private_key.take());
        self.polymarket.proxy_wallet_address = std::env::var("POLYMARKET_PROXY_WALLET_ADDRESS")
            .ok()
            .or_else(|| self.polymarket.proxy_wallet_address.take());
        self.polymarket.signature_type = std::env::var("POLYMARKET_SIGNATURE_TYPE")
            .ok()
            .and_then(|v| v.parse::<u8>().ok())
            .or(self.polymarket.signature_type);
    }
}
