...
pub fn log_to_history(message: &str) {
    ...
    let plain = polymarket_trading_bot::term_ui::strip_ansi_for_file(message);
    if let Some(file_mutex) = HISTORY_FILE.get() {
        if let Ok(mut file) = file_mutex.lock() {
            let _ = write!(file, "{}", plain);
            let _ = file.flush();
        }
    }
}
...
fn run_trailing_stop_replay(
    ...
    eprintln!();
    polymarket_trading_bot::term_ui::print_section_title("Trailing-stop replay (Up filled @ 0.45, hedge Down)");
    eprintln!("  Snapshots: {}", snapshots.len());
    if let Some((kind, elapsed, price)) = result {
        eprintln!(
            "  Would hedge: {} @ {}s ({}m {}s)  price ${:.4}",
            kind,
            elapsed,
            elapsed / 60,
            elapsed % 60,
    ...
    polymarket_trading_bot::term_ui::print_rule_dim();
    eprintln!();
}
...
async fn main() -> Result<()> {
    ...
    polymarket_trading_bot::term_ui::print_startup_banner(is_simulation, limit_price, "history.toml");
    ...
    let shares_line = if let Some(shares) = limit_shares {
        format!("Shares per order (config): {:.6}", shares)
    } else {
        "Shares per order: fixed_trade_amount / price".to_string()
    };
    let markets_line = format!(
        "Markets: BTC + {}",
    ...
    polymarket_trading_bot::term_ui::print_strategy_config(
        limit_price,
    ...
    let api = Arc::new(PolymarketApi::from_config(&config));

    polymarket_trading_bot::term_ui::print_auth_start();
    api.authenticate().await.context("Authentication failed")?;

    if is_simulation {
        polymarket_trading_bot::term_ui::print_simulation_hint();
    }

    polymarket_trading_bot::term_ui::print_section_title("Market discovery");
    ...
    let sync_banner = polymarket_trading_bot::term_ui::format_portfolio_sync_banner();
    crate::log_to_history(&format!("{}\n", sync_banner));
    ...
    monitor_arc.start_monitoring(move |snapshot| {
        ...
        let _previous_prices = previous_prices.clone();
        ...
        async move {
            ...
            if opportunities.is_empty() {
                ...
                if !checked_opportunities.is_empty() {
                    ...
                    let mut signer_mismatch_detected = false;
                    for batch_attempt in 1..=3 {
                        match trader.execute_limit_buy_batch(
                                ...
                                for (opp, resp) in checked_opportunities.iter().zip(responses.iter()) {
                                    ...
                                    if success {
                                        ...
                                        if resp
                                            .message
                                            .as_deref()
                                            .map_or(false, |m| m.contains("invalid: signer"))
                                        {
                                            signer_mismatch_detected = true;
                                        }
                            ...
                            Err(e) => {
                                ...
                                if e.to_string().contains("invalid: signer") {
                                    signer_mismatch_detected = true;
                                }
                                if batch_attempt == 3 {
                    ...
                    if signer_mismatch_detected {
                        warn!("🚫 CLOB rejected signer for this wallet/proxy pair. Skipping per-order retries to avoid spam. Fix signer/proxy mapping, then restart.");
            ...
            if is_simulation {
                ...
                if let Some(simulation_tracker) = simulation_tracker_opt {
                    ...
                    if should_log_trends {
                        ...
                        crate::log_println!(
                            "{}",
                            polymarket_trading_bot::term_ui::trend_banner(
                        ...
                        crate::log_println!(
                            "{}",
                            polymarket_trading_bot::term_ui::trend_banner_end()
                        );
                    }
                }
            }
            ...
            // SIM status: avoid printing every snapshot for the first half-minute (too noisy next to price ticks).
            if is_simulation
                && (time_elapsed_seconds % 15 == 0
                    || (time_elapsed_seconds <= 30 && time_elapsed_seconds % 5 == 0))
            {
                ...
                for (market_name, condition_id, up_token, down_token, up_type, down_type) in &sim_markets {
                    ...
                    crate::log_println!("{}", polymarket_trading_bot::term_ui::sim_divider());
                    crate::log_println!(
                        "{}",
                        polymarket_trading_bot::term_ui::format_sim_head(
                            market_name,
                            time_elapsed_seconds / 60,
                            time_elapsed_seconds % 60
                        )
                    ...
                    crate::log_println!(
                        "{}",
                        polymarket_trading_bot::term_ui::format_sim_prices(
                            up_bid,
                            up_ask,
                            up_limit_fills,
                            down_bid,
                    ...
                    crate::log_println!(
                        "{}",
                        polymarket_trading_bot::term_ui::format_sim_fills(up_filled, down_filled)
                    );
                    if up_filled != down_filled {
                        ...
                        if already_hedged_2min {
                            crate::log_println!(
                                "{}",
                                polymarket_trading_bot::term_ui::format_sim_tag_line(
                                    market_name,
                                    &format!(
                                        "One side filled → unfilled {} hedged (2-min market buy done).",
                                        unfilled_name
                                    ),
                                )
                            );
                            ...
                            crate::log_println!(
                                ...
                                polymarket_trading_bot::term_ui::format_sim_tag_line(
                                    market_name,
                                    &format!(
                                        "One side filled. Unfilled {} bid=${:.4} ask=${:.4}. Lowest=${:.4}, hedge when price >= lowest+{:.3}={:.4}. Would trigger now: {}",
                                        unfilled_name,
                                        unfilled_bid,
                                        unfilled_ask,
                                        lowest,
                        ...
                        crate::log_println!(
                            ...
                            polymarket_trading_bot::term_ui::format_sim_tag_line(
                                market_name,
                                "Both sides filled at 0.45 (no hedge).",
                            )
                        );
                    } else {
                        crate::log_println!(
                            ...
                            polymarket_trading_bot::term_ui::format_sim_tag_line(
                                market_name,
                                "Waiting for limit fills (both unfilled).",
                            )
                        );
                    }
                }
                ...
                for (market_name, condition_id, up_token, down_token, up_type, down_type) in &markets_to_check {
                    ...
                    if current_ask >= band_threshold {
                        ...
                        if lowest_before <= bounce_threshold {
                            ...
                            let mut map = two_min_trailing_min_ask.lock().await;
                            if map.get(&price_key).copied() != Some(current_ask) {
                                map.insert(price_key.clone(), current_ask);
                                drop(map);
                                crate::log_println!(
                    ...
                    // Trailing stop: track lowest ASK (we buy at ask). Trigger when current_ask >= lowest_ask + hedge_trailing_stop (ask bounced from low; we buy at current ask).
                    {
                        ...
                        if time_elapsed_seconds < NINETY_SEC_AFTER_SECONDS && current_ask >= LIMIT_PRICE {
                            ...
                            if new_min_ask <= bounce_threshold {
                                ...
                                if min_map.get(&price_key).copied() != Some(current_ask) {
...
async fn get_or_discover_markets(
    ...
    let eth_market = if enable_eth {
        polymarket_trading_bot::term_ui::print_discover_section_start("ETH");
    ...
    polymarket_trading_bot::term_ui::print_discover_section_start("BTC");
...
async fn discover_solana_market(
    ...
    polymarket_trading_bot::term_ui::print_discover_section_start("Solana");
...
async fn discover_xrp_market(
    ...
    polymarket_trading_bot::term_ui::print_discover_section_start("XRP");
...
async fn discover_market(
    ...
    for (i, prefix) in slug_prefixes.iter().enumerate() {
        if i > 0 {
            polymarket_trading_bot::term_ui::print_discover_alt_slug_prefix(market_name, prefix);
        }
        ...
        if let Ok(market) = api.get_market_by_slug(&slug).await {
            if !seen_ids.contains(&market.condition_id) && market.active && !market.closed {
                polymarket_trading_bot::term_ui::print_market_found(
                    market_name,
                    &market.slug,
                    &market.condition_id,
                );
        ...
        if include_previous {
            for offset in 1..=3 {
                ...
                polymarket_trading_bot::term_ui::print_discover_try_previous_slug(market_name, &try_slug);
