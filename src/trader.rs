                ...
                Ok(response) => {
                    ...
                    let balance_f64 = if is_hedge_trade {
                        ...
                        match self.api.check_balance_allowance(&opportunity.token_id).await {
                            ...
                            Err(_) => units, // Fallback to expected
                        }
                    };
        ...
        if self.simulation_mode {
            if let Some(tracker) = &self.simulation_tracker {
                ...
                for position in positions {
                    ...
                    let _seconds_until_close = market_end_timestamp.saturating_sub(current_timestamp);
        ...
        for (key, trade) in pending_trades {
            ...
            if market_closed {
                ...
                *total += profit;
                ...
                // Redemption failures use `continue` above; reaching here means success or simulation.
