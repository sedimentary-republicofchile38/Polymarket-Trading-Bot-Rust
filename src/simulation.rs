...
impl SimulationTracker {
    ...
    pub async fn log_trend_analysis(
        ...
        if let Some(trend) = self.get_trend_analysis(period_timestamp, token_id, min_samples).await {
            let direction_key = match trend.direction {
            ...
            let plain = format!(
                "TREND {} | {} | str {:.3} | d_price {:.4} | slope {:.6}/s | {}s | n={}",
                full_name,
                direction_key,
                trend.strength,
                trend.price_change,
            ...
            self.log_to_file(&plain).await;
            eprintln!(
                "{}",
                crate::term_ui::format_trend_row(
                    &full_name,
                    direction_key,
                    trend.strength,
                    trend.price_change,
            ...
            let plain = format!(
                ...
                full_name, min_samples
            );
            self.log_to_file(&plain).await;
            eprintln!(
                "{}",
                crate::term_ui::format_trend_insufficient(&full_name, min_samples)
            );
        }
    }
    ...
    async fn fill_limit_order(&self, order_key: &str, current_prices: &HashMap<String, TokenPrice>) {
        ...
        match order.side.as_str() {
            "BUY" => {
                ...
                let (total_spent, _total_earned, total_realized_pnl) = self.get_total_spending_and_earnings().await;
